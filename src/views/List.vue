...
const jumpLink = (data) => {
  ...
  const isMobile = typeof window !== "undefined" ? window.innerWidth <= 680 : false;
  const url = isMobile ? data.mobileUrl : data.url;
...
watch(
  ...
  (val) => {
    if (val.name === "list") {
      listType.value =
        val.query.type || store.newsArr[0]?.name || listType.value;
...
onMounted(() => {
  getHotListsData(listType.value);
  refreshRelativeTime();
  relativeTimer = setInterval(refreshRelativeTime, 60000);
});
