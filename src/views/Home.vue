...
import { warmupHotList } from "@/api/hotListCache";

const store = mainStore();
const PREFETCH_COUNT = 4;

const prefetchHotLists = () => {
  const visibleItems = store.newsArr
    .filter((item) => item.show)
    .slice(0, PREFETCH_COUNT);
  visibleItems.forEach((item) => {
    warmupHotList({ name: item.name, params: item.params });
  });
};

watch(
  () =>
    store.newsArr.map((item) => `${item.name}-${item.show}`).join("|"),
  () => {
    if (store.newsArr.length) {
      prefetchHotLists();
    }
  },
  { immediate: true }
